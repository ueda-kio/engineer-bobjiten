...
**リセットは難易度ごとに独立して判定する。** ある難易度で候補を抽選できなくなった時点で、**その難易度の出題済み記録のみ**を消去して再利用を許可する。

全難易度をまとめて消去しない理由は、語の在庫が難易度間で均等とは限らないためである。先に尽きた難易度の巻き添えで、まだ余っている難易度の履歴まで消えると、リセットの頻度が不必要に上がる。データを増やしても偏りがある限り同じ問題が残る。

**リセットが発生したことを画面に表示する。** 通知がないと、既出の語が再び現れた理由が参加者に分からず混乱する。どの難易度で一巡したかを併せて示す。
...
- **ホストまたは現在の出題者:** 結果表示から次へ進む
- **現在の出題者のみ:** お題を選択、引き直し、お助け機能を使用、正解者を確定、カタカナ使用の指摘を認める
- **参加者本人:** ルームへの入室

「次へ」をホストだけに限ると、ホストが会話に入っている間ゲーム全体が止まる。進行を止めない主体を2つ用意する。

**カタカナ使用の指摘そのものは口頭で行う。** アプリ上の操作は 5.2 のとおり「出題者が違反を認める」ことであり、実行主体は出題者である。回答者が自分で加点できる形にしてはならない。
